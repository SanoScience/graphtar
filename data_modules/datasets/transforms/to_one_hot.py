...
class ToOneHot(JSONSerializable, object):
    ...
    def __call__(self, sample):
        ...
        sample[self.key_to_encode] = np.array(
            [self.one_hot_encode(word) for word in divided]
        )
        return sample
    ...
    def one_hot_encode(self, word):
        return np.array(
            [
                np.eye(max(self.encoding_dict.values()) + 1)[self.encoding_dict[char]]
                for char in word
            ]
        )
    ...
    def to_json(self):
        return {
            ...
            "key_to_encode": self.key_to_encode,
        }
