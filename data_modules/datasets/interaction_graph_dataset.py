...
class InteractionGraphDataset(InteractionDataset):
    ...
    def __getitem__(self, idx):
        ...
        data_objects = []
        for mirna, target, label in zip(
            sample[self.mirna_key], sample[self.target_key], sample[self.label_key]
        ):
    ...
    def get_edges(self, mirna, target):
        ...
        cross_edges = np.array(
            (np.arange(shorter_len), np.arange(shorter_len) + shorter_len)
        )
        cross_edges = np.concatenate((cross_edges, cross_edges[[1, 0], :]), axis=1)
        cross_edges_features = np.tile([0, 1], (cross_edges.shape[1], 1))
        edges = np.concatenate(
            (mirna_inter_edges, target_inter_edges, cross_edges), axis=1
        )
        edge_features = np.concatenate(
            (
                mirna_inter_edges_features,
                target_inter_edges_features,
                cross_edges_features,
            ),
            axis=0,
        )
        return torch.as_tensor(edges, dtype=torch.long), torch.as_tensor(
            edge_features, dtype=torch.long
        )

    def get_inter_edges(self, sequence):
        inter_edges = np.array(
            (np.arange(len(sequence))[:-1], np.arange(len(sequence))[:-1] + 1)
        )
