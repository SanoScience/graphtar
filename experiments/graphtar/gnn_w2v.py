...
data_module = GraphInteractionDataModule(
    config_path, int(batch_size), int(data_split_seed), transform=transform
)

graph_layer_sizes = [
    (16, hyperparams["gnn_hidden_size"]),
    *[
        (hyperparams["gnn_hidden_size"], hyperparams["gnn_hidden_size"])
        for i in range(hyperparams["n_gnn_layers"] - 1)
    ],
]
fc_layer_sizes = [
    (hyperparams["gnn_hidden_size"], hyperparams["fc_hidden_size"]),
    *[
        (hyperparams["fc_hidden_size"], hyperparams["fc_hidden_size"])
        for i in range(hyperparams["n_fc_layers"] - 1)
    ],
]

module = GnnLM(
    LayerType[gnn_layer_type],
    graph_layer_sizes,
    GlobalPoolingType[global_pooling],
    fc_layer_sizes,
...
trainer = Trainer(
    gpus=1,
    max_epochs=hyperparams["epochs_num"],
    callbacks=[
